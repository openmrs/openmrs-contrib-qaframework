...
public class AppointmentBlocksPage extends Page {
	...
	private static final By START_TIME = By.cssSelector("#start-time table");
	...
	private static final By PROVIDER = By.cssSelector("#select-provider input");
	...
	private static final By DAY = By.cssSelector("#calendar table tbody tr td.fc-header-left span.fc-button.fc-button-basicDay.fc-state-default.fc-corner-left.fc-corner-right");
	private static final By DAY_CLASS = By.className("fc-day-content");
	private static final By LOCATION_IN_BLOCK = By.cssSelector("#filter-location select");
	private static final By DELETE_CONFIRM = By.cssSelector("#delete-appointment-block-modal-buttons .confirm");
	private static final By SERVICE_DELETE = By.cssSelector("#filter-parameters > selectmultipleappointmenttypes > div > div.selectmultipleWidget > div.selectmultipleBox > div.selectedAppointmentTypesList.ng-scope > i");
	...
	private static final By DAY_BLOCK = By.className("fc-day-number");
	private static final By REMOVE_APPOINTMENT = By.linkText("Delete");
	...
	/**
	 * Removes an appointment. Before calling this method is necessary to click
	 * on the day (User should click on the day button on the left)
	 * 
	 * @see #clickOnDay()
	 */
	...
	public void selectLocation(String location) {
		// a wait might be needed if there are many elements to load in the
		// dropdown menu
	 ...
	 * is hidden by the tooltip. An alternative solution to this method is to
	 * create a method that closes the tooltip such as: private static final By
	 * CLOSE_TOOLTIP = By.cssSelector("span.ui-icon.ui-icon-close"); public void
	 * clickOnCloseTooltip(){ clickOn(CLOSE_TOOLTIP); } Unfortunately this
	 * method did not prove to be always reliable (sometime Selenium fails to
	 * locate span.ui-icon.ui-icon-close)
	 */
	...
	/**
	 * Clicks on the current day and open the "editing page". This method should
	 * be invoked when the page shows only a block with the day
	 */
	 ...
	 * 
	 * @param serviceChoice
	 *            , the index of the service. For instance if there are 2 rows
	 *            on the block, to get the first one serviceChoice = 0
