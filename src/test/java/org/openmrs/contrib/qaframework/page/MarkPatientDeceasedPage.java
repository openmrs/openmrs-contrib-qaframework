...
public class MarkPatientDeceasedPage extends Page {
	...
	private static final By ADD_NEW_CONCEPT_LINK = By
			.xpath("//*[@id=\"content\"]/a[2]");

	private static final By CONCEPT_NAME = By.id("namesByLocale[en].name");

	private static final By ADD_ANSWER_BUTTON = By
	...
	private static final By FIRST_CAUSE_OPTION_LINK = By
			.className("autocompleteresult");

	private static final By ADD_BUTTON = By.className("ui-button-text");

	private static final By SAVE_CONCEPT_BUTTON = By
			.cssSelector("#saveDeleteButtons > input[type=submit]:nth-child(1)");

	private static final By SEARCH_CONCEPT_FIELD = By.id("inputNode");

	private static final By FIRST_RESULT_LINK = By.className("odd");

	private static final By CONCEPT_ID_VALUE = By
			.cssSelector("#conceptTable > tbody > tr:nth-child(1) > td");

	private static final By ADMINISTRATION_PAGE = By
			.id("administrationNavLink");

	private static final By ADVANCED_SETTINGS_LINK = By
			.linkText("Advanced Settings");

	private static final By CONCEPT_VALUE_FIELD = By
			.cssSelector("#globalPropsList > tr:nth-child(133) > td:nth-child(2) > input[type=text]");

	private static final By SAVE_CONCEPT_VALUE = By.name("action");

	private static final By HOME_LINK = By.id("homeNavLink");

	private static final By MARK_PATIENT_DECEASED_LINK = By
			.id("org.openmrs.module.coreapps.markPatientDead");
	...
	private static final By SAVE_DECEASED_BUTTON = By
	...
	private static final By DELETE_BUTTON = By
			.cssSelector("#saveDeleteButtons > input[type=submit]:nth-child(4)");
	...
	/*
	 * If there is a concept called Cause Of Death, The method gets its concept
	 * code if there is no concept at all or no concept called Cause of death,
	 * the method creates one and copies its concept code
	 */
	...
	public void newConcept() {
		...
		waiter.until(ExpectedConditions
				.elementToBeClickable(SAVE_CONCEPT_BUTTON));
